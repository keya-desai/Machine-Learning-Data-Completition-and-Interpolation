...
def dev_model(X_prime_train, X_prime_test, isFeatureReal_train, isFeatureReal_test, baseline_model_obj):
    ...
    for idx, row in isFeatureReal_train.loc[isFeatureReal_train[col_to_predict] == 0].iterrows(): 
        # print(col_to_predict)
        # print(row)
        # print(len(row))
        # print(row.drop(col_to_predict))
        # new_row = 
    ...
    new_w = np.delete(w, col_to_predict)
    for idx, row in isFeatureReal_test.loc[isFeatureReal_test[col_to_predict] == 0].iterrows():
        new_row = row.drop(col_to_predict)
        pred = - (np.dot(new_w, new_row) + b )/w[col_to_predict]
