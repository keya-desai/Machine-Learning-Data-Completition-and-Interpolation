...
def model(X_prime_train, X_prime_test, isFeatureReal_train, isFeatureReal_test, categoricalFeatures):
    ...
    x_testing = x_training[x_training[first_col].isnull()]
    ...
    for col_to_predict in sorted_index_list[1:]:
        ...
        if col_to_predict not in categoricalFeaturesFlatten:
            X_prime_train[:, col_to_predict], X_prime_test[:, col_to_predict] = predictContinuousFeature(pd.DataFrame(X_prime_train), pd.DataFrame(X_prime_test), isFeatureReal_train, isFeatureReal_test, col_to_predict, predicted_indexes, x_testing)    
...
def predictContinuousFeature(train_data, test_data, isFeatureReal_train, isFeatureReal_test, col_to_predict, predicted_indexes, x_testing):
